#!/usr/bin/env python3
"""
Test script to verify skills appear after reenlistment
"""

import character_generation_rules as chargen

def test_skills_after_reenlistment():
    """Test that skills are available after reenlistment"""
    print("Testing Skills After Reenlistment")
    print("=" * 40)
    ...
    if reenlistment_result.get('continue_career', False):
        # Check skills after reenlistment
        print("\n4. Skills After Reenlistment...")
        available_tables = chargen.get_available_skill_tables(character)
        skill_eligibility = character.get("skill_eligibility", 0)
        ready_for_skills = chargen.is_ready_for_skills(character)
        print(f"Skill eligibility: {skill_eligibility}")
        ...
        if ready_for_skills and skill_eligibility > 0:
            print("✅ SUCCESS: Skills are available after reenlistment!")
        else:
            print("❌ FAILURE: Skills are not available after reenlistment!")
            print(f"   Ready for skills: {ready_for_skills}")
            print(f"   Skill eligibility: {skill_eligibility}")
    else:
...
if __name__ == "__main__":
    test_skills_after_reenlistment() 